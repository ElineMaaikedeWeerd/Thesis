...
%\\
% {A} &=& {b_{\rm e}+{3\over2}\Omega_m -3 + (2-5s) \Big(1-{1\over r\cH} \Big)}\,.
\end{equation}
...
Although the galaxy density contrast \eqref{dopf2} is complex, the power spectrum is real:
\begin{equation}
...
\bea
&&\delta^{(2)}_{g{\rm D}}=A\, \bm{v}^{(2)}\!\!\cdot\n+2{C}(\bm{v}\cdot\n)\delta +2{{E}\over\cH}(\bm{v}\cdot\n)\partial_r(\bm{v}\cdot\n)\label{dg2}
\\&&\nonumber
+2{b_1\over\cH}\phi\, \partial_r\delta
+{2\over\cH^2}\big[\bm{v}\cdot\n \,\partial_r^2\phi-\phi\, \partial_r^2 (\bm{v}\cdot\n) \big] -{2\over \cH}\partial_r (\bm{v}\cdot\bm{v}), 
\eea
...
P(k_{1})P(k_{2}) \nonumber\\&&+\text{2 cyclic permutations}\,.\label{b6}
\eea
...
\bea
%\kt(\ka,\kb,\kc) &=&\kt_{\rm N}(\ka,\kb,\kc)+ \kt_{\rm D}(\ka,\kb,\kc) \label{d1}\\
&&\mathcal{K}_{\rm{N}}^{(2)}%(\mathbf{k}_{1},  \mathbf{k}_{2},\k_3) 
...
\bigg[-{3\over 2}\left(\mu_{1}{k_{1}\over k_2^2} + \mu_{2}{k_{2}\over k_1^2}\right)\Omega_m b_1 +2\mu_{12} \left({\mu_{1}\over k_2} + {\mu_{2} \over k_1}\right)f^2  + \left({\mu_{1}\over k_1} + {\mu_{2} \over k_2}\right){C} f  
\nonumber \\
&&~~~~~ -{3\over 2} \left(\mu_{1}^3{k_{1}\over k_2^2} + \mu_{2}^3{k_{2}\over k_1^2}\right)\Omega_mf 
 + \mu_{1}\mu_{2}\left({\mu_{1}\over k_2} + {\mu_{2} \over k_1}\right){\Big({3\over2}\Omega_m-E f\Big) f} + {\mu_{3} \over k_{3}}G_{2}(\bm{k}_{1}, \bm{k}_{2},\kc){A} f\bigg] ,~~~~  \label{d2}
\eea
...
we find that
\bea 
B_{g{\rm N}}( \mathbf{k}_{1},  \mathbf{k}_{2},  \k_3) &=&  \ko_{\rm N}(\ka)\ko_{\rm N}(\kb)\kt_{\rm N}(\ka,\kb,\kc)\,P(k_{1})P(k_{2})  +\text{2 cyclic permutations} \,,\\
B_{g{\rm D}}( \mathbf{k}_{1},  \mathbf{k}_{2},  \k_3) &=& \Big\{
...
\label{k2d1} 
% {\cal K}^{(2)}_{\rm D}(\k_{-S},\k_L,\k_S) &=&  {\cal K}^{(2)}_{\rm D}(\k_L,\k_S,\k_{-S})\Big|_{\mu_S\to \roy{\mu_{-S}}}  \label{k2d2}
\eea
...
\bea
%B_{g}^{\rm sq}
%&=& B_{g {\rm N}}^{\rm sq}+B_{g {\rm D}}^{\rm sq} \nonumber\\
%B_{g {\rm N}}^{\rm sq} &=& b_{1S}^2\Big(b_2-{4\over21}b_1+{4\over 21} \Big)P_S^2+b_{1S}b_{1L}b_{SL}\, P_LP_S\,,\nonumber
&&B_{g {\rm}}^{\rm sq} = b_{1S}b_{1L}b_{SL}\, P_LP_S+\i b_{1S}\Big\{b_{SL}f A + 
%\big[-\big( \beta_{10}+3\mu_S^2 \beta_{13}\big)+ 2\big( \beta_{12}+\mu_S^2 \beta_{14}\big)\big] 
{3\over2}\Omega_m b_{1S}b_{1L} 
\nonumber\\&&+2b_{1L}f C+ b_{1L}\mu_S^2\Big[{3\over2}\Omega_m- Ef \Big]}
\Big\}P_LP_S\,\mu_L{{\cH}\over k_L \,,\label{bgd}
 \eea
where $P_{S,L}=P(k_{S,L})$, $b_{1S,L}\equiv  b_1+f\mu_{S,L}^2$ and
\bea
% \,, \label{b1sl}~~
b_{SL} \equiv 2b_2+ {43\over 21}b_1-{4\over21}
%\nonumber\\&&
+\Big(2b_1+ {5\over7}\Big)f\mu_S^2%+b_1f\mu_L^2+{f^2}\mu_S^2\mu_L^2 
+f\mu_L^2 b_{1S}
\,. \nonumber\label{bsl}
\eea
%Note that we can neglect the $P(k_S)^2$ term relative to the $P(k_L)P(k_S)$ term in $B_{g {\rm N}}^{\rm sq}$.
...
\includegraphics[width=\columnwidth]{figuresv2-02}
...
\includegraphics[width=\columnwidth]{figuresv2-01}
