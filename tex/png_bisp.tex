...
& \times \left[\partial_{\parallel}V^{(2)}_{\mathrm{N+GR}} -\Phi^{(2)}_{\mathrm{N+GR}} \right] + 2 (\mathcal{Q}-1)\Psi^{(2)}_{\mathrm{N+GR}}  +\Phi^{(2)}_{\mathrm{N+GR}} + \frac{1}{\mathcal{H}}\Psi^{(2)\prime}_{\mathrm{N+GR}} \nonumber \\
