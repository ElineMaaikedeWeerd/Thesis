...
\iffalse
\be \label{kfgpar}
k_{\| \rm fg} = 0.01 \,h\,{\rm Mpc}^{-1},
\ee
{but we will also consider an optimistic case, k$_{\| \rm fg} = 0.005 \,h\,{\rm Mpc}^{-1}$, that anticipates further developments of the reconstruction technique.}
...
\big|k_{\parallel}\big| > k_{\rm wedge}(z)\,k_{\perp} \qquad \mbox{for IF survey mode,}
\end{equation} 
where $k_\perp=\sqrt{1-\mu^2}\,k$ and  $k_{\rm wedge}$ is modelled as
\begin{equation}
k_{\rm wedge}(z) = r(z)\cH(z) \sin{\big[0.61N_{\mathrm{w}}\,{\theta_{\rm b}(z)}\big]} \, . \label{e2.20} 
\end{equation}
{The beam (defining the IF field of view) is given for a dish array by
\be\label{thetab}
\theta_{\rm b}(z) = 1.22 \,{\lambda(z) \over D_{\rm d}}\,,
\ee
where $\lambda(z)=\lambda_{21}(1+z)$ and $D_{\rm d}$ is the dish diameter.
